  ...
  return (
    <Animated.View style={[styles.scrapbookCard, animatedCardStyle]}>
      ...
      >
        ...
        >
          <Text style={styles.scrapbookTitle} numberOfLines={1}>{scrapbook.title}</Text>
          <View style={styles.scrapbookMetaRow}>
            <View style={styles.scrapbookMetaItem}>
              <Ionicons name="images-outline" size={14} color="#9575CD" />
              <Text style={styles.scrapbookMetaText}>{scrapbook.imageCount}</Text>
            </View>
            <View style={styles.scrapbookMetaItem}>
              <Ionicons name="people-outline" size={14} color="#9575CD" />
              <Text style={styles.scrapbookMetaText}>{scrapbook.collaborators}</Text>
            </View>
            <View style={styles.scrapbookMetaItem}>
...
const ProfileScreen = ({ navigation }) => {
  ...
  useEffect(() => {
    statsSectionOpacity.value = withDelay(300, withTiming(1, { duration: 800 }));
    ...
    try {
      ...
      if (!result.canceled && result.assets && result.assets[0]) {
        ...
        avatarScale.value = withSequence(
          ...
          withTiming(1, { duration: 200, easing: Easing.bezierFn(0.25, 0.1, 0.25, 1) })
        );
  ...
  const myScrapbooks = scrapbooks?.filter(s => s.owner._id === userData?._id) || [];
  const collaboratingScrapbooks = scrapbooks?.filter(s => s.owner._id !== userData?._id) || [];
  ...
  const handleLogoutModalOpenClose = () => {
    ...
    logoutModalOpacity.value = withTiming(!logoutModalVisible ? 1 : 0, { duration: 300 });
  }
  ...
  return (
    <View style={styles.container}>
            ...
            <Animated.View style={[styles.profileInfo, avatarAnimatedStyle]}>
                ...
                >
                  {userData?.avatar ? (
                    ...
                    <View style={[styles.avatarPlaceholder, { zIndex: 100 }]}>
                      <MaterialCommunityIcons name="face-man-outline" size={AVATAR_SIZE * 0.5} color="#9575CD" />
                    </View>
                  )}
                </LinearGradient>
              </TouchableOpacity>
              ...
              {isEditingUsername ? (
                <View style={styles.usernameEditContainer}>
                  <BlurView style={styles.usernameEditBlur} intensity={40} tint="dark">
                    <TextInput
                ...
                >
                  ...
                  <Ionicons name="create-outline" size={18} color="#9575CD" style={styles.editIcon} />
                </TouchableOpacity>
              )}
            </Animated.View>
          </View>
                ...
                <View style={styles.scrapbooksSection}>
                  <View style={styles.sectionHeader}>
                    ...
                    <View style={styles.sectionBadge}>
                      <Text style={styles.sectionBadgeText}>{myScrapbooks.length}</Text>
                    </View>
                  </View>
                  ...
                  <View style={styles.scrapbooksGrid}>
                    {myScrapbooks.map(scrapbook => (
                      <ScrapbookCard
                        ...
                        onPress={() => navigation.navigate('ScrapbookEditor', {
              ...
              {collaboratingScrapbooks.length > 0 && (
                <View style={styles.scrapbooksSection}>
                  <View style={styles.sectionHeader}>
                    ...
                    <View style={styles.sectionBadge}>
                      <Text style={styles.sectionBadgeText}>{collaboratingScrapbooks.length}</Text>
                    </View>
                  </View>
              ...
              {scrapbooks?.length === 0 && (
                <View style={styles.emptyStateContainer}>
                  <View style={styles.emptyStateIconContainer}>
                    <BlurView style={styles.emptyStateIconBlur} intensity={20} tint="dark">
                      <Ionicons name="book-outline" size={40} color="#9575CD" />
                    </BlurView>
                  </View>
                  <Text style={styles.emptyStateTitle}>No Scrapbooks Yet</Text>
                  <Text style={styles.emptyStateText}>Create your first scrapbook and start collecting memories</Text>
                  <TouchableOpacity style={styles.emptyStateButton} onPress={handleCreateScrapbook}>
                    <LinearGradient
                      colors={['#5C6BC0', '#9575CD']}
                    ...
                    >
                      <Text style={styles.emptyStateButtonText}>Create Scrapbook</Text>
                    </LinearGradient>
                  </TouchableOpacity>
                </View>
              )}
      ...
      {scrapbooks?.length > 0 && (
        <Animated.View style={[styles.fab, fabAnimatedStyle, { bottom: insets.bottom + 20 }]}>
          <TouchableOpacity 
